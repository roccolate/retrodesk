...
RetroFsError retro_fs_path_init(RetroFsPath *p, const char *v) {
    ...
    p->value = strdup(v); return p->value ? RETRO_FS_OK : RETRO_FS_OOM;
}
void retro_fs_path_destroy(RetroFsPath *p) { if (p) { free(p->value); p->value=NULL; } }
const char *retro_fs_path_cstr(const RetroFsPath *p) { return p ? p->value : NULL; }
RetroFsError retro_fs_path_parent(const RetroFsPath *p, RetroFsPath *out) {
    if (!p || !p->value || !out) return RETRO_FS_INVALID_ARGUMENT;
    char *copy=strdup(p->value); if (!copy) return RETRO_FS_OOM;
...
RetroFsError retro_fs_path_join(const RetroFsPath *b,const char *name,RetroFsPath *o) {
    if (!b||!b->value||!name||!o||strchr(name,'/')) return RETRO_FS_INVALID_ARGUMENT;
    size_t n=strlen(b->value), m=strlen(name); char *s=malloc(n+m+2);
    if (!s) return RETRO_FS_OOM;
    memcpy(s,b->value,n); s[n]='/'; memcpy(s+n+1,name,m+1); o->value=s;
    return RETRO_FS_OK;
}
RetroFsError retro_fs_stat(const RetroFsPath *p, RetroFsVersion *v) {
    struct stat s; if (!p||!p->value||!v) return RETRO_FS_INVALID_ARGUMENT;
    if (stat(p->value,&s)<0) return map_errno(errno);
    fill_version(&s,v); return RETRO_FS_OK;
}
...
RetroFsError retro_fs_list(const RetroFsPath *p, size_t max_entries,
    ...
    if (!p || !p->value || !cb || max_entries == 0) return RETRO_FS_INVALID_ARGUMENT;
...
RetroFsError retro_fs_read_text(const RetroFsPath *p,char **out,size_t *len,RetroFsVersion *v) {
    int fd; struct stat s; size_t got=0; char *buf;
    if (!p||!p->value||!out||!len) return RETRO_FS_INVALID_ARGUMENT;
    fd=open(p->value,O_RDONLY); if(fd<0)return map_errno(errno);
    if(fstat(fd,&s)<0){int e=errno;close(fd);return map_errno(e);} if(!S_ISREG(s.st_mode)){close(fd);return RETRO_FS_UNSUPPORTED;} if((uintmax_t)s.st_size>RETRO_FS_MAX_TEXT){close(fd);return RETRO_FS_TOO_LARGE;}
    buf=malloc((size_t)s.st_size+1); if(!buf){close(fd);return RETRO_FS_OOM;}
    while(got<(size_t)s.st_size){ssize_t n=read(fd,buf+got,(size_t)s.st_size-got);if(n<=0){free(buf);close(fd);return RETRO_FS_IO;}got+=(size_t)n;} close(fd); buf[got]=0;
    bool saw_lf = false;
    bool saw_crlf = false;
    for (size_t i = 0; i < got; ++i) {
        unsigned char c = (unsigned char)buf[i];
        if (c == 0 || c == 27 || (c < 32 && c != '\t' && c != '\n' && c != '\r') ||
            c >= 127) {
            free(buf);
    ...
    *out=buf;*len=got;if(v)fill_version(&s,v);return RETRO_FS_OK;
}
RetroFsError retro_fs_write_atomic(const RetroFsPath *p, const char *data, size_t n,
    ...
    if (exists && (!S_ISREG(old.st_mode) || S_ISLNK(old.st_mode))) return RETRO_FS_UNSUPPORTED;
    if (exp && (!exists || !same_version(exp, &(RetroFsVersion){old.st_dev, old.st_ino,
                                                                  old.st_size, old.st_mtime,
                                                                  old.st_mode}))) {
    ...
    size_t w=0;while(w<n){ssize_t x=write(fd,data+w,n-w);if(x<=0){close(fd);unlink(tmp);free(tmp);retro_fs_path_destroy(&par);return RETRO_FS_IO;}w+=(size_t)x;}if(fsync(fd)<0){close(fd);unlink(tmp);free(tmp);retro_fs_path_destroy(&par);return RETRO_FS_IO;}close(fd);if(rename(tmp,p->value)<0){int e=errno;unlink(tmp);free(tmp);retro_fs_path_destroy(&par);return map_errno(e);}int dfd=open(par.value,O_RDONLY|O_DIRECTORY);if(dfd>=0){fsync(dfd);close(dfd);}free(tmp);retro_fs_path_destroy(&par);if(written)retro_fs_stat(p,written);return RETRO_FS_OK;
}
const char *retro_fs_error_string(RetroFsError e){static const char *s[]={"ok","not found","permission denied","unsupported","too large","invalid text","conflict","out of memory","I/O error","invalid argument"};return e<=RETRO_FS_INVALID_ARGUMENT?s[e]:"unknown";}
