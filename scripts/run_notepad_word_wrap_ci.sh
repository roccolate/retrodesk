...
run_logged() {
    ...
    if ! "$@" >"$log" 2>&1; then
        ...
        cp "$log" "ci-artifacts/notepad-word-wrap-${label}.log"
        local summary
...
run_logged repair python3 scripts/fix_word_wrap_test_helper.py
run_logged apply python3 scripts/apply_notepad_word_wrap.py
