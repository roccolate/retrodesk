...
- `apps` depend on app/runtime, UI widgets, and high-level storage contracts.
...
The active product slice is Linux/POSIX:

- File Manager and Notepad use the storage contract for directory listing,
  text reads, and atomic saves.
- Diagnostics is a hosted app but intentionally does not provide a shell.
- Native Windows, macOS, and DOS require storage adapter work or app gating
  before they can claim the same app surface.
